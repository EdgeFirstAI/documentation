...
- **Clock Synchronization and Timestamps page** (`perception/clock_sync.md`): the three timestamps carried by every middleware message (`header.stamp`, the Zenoh sample timestamp, and MCAP `publish_time`/`log_time`), the Linux clocks behind them, and the acquisition-time contract for source publishers, derived publishers, the Recorder, the dataset Publisher and Replay, including how a clock step inside a recording is marked with `clock_step` and `clock_sync` metadata and handled as a segment boundary.  Documents the Maivin time sources (no persistent RTC, chrony with GNSS PPS and NTP, optional PTP for sensors), the boot sequence, resilience to late or absent synchronization, a per-topic service reference, a recording inspection script and troubleshooting (DE-2964).
- **Profiling the Middleware** (`perception/profiling.md`): the Tracy-based workflow for the perception services, covering which services are instrumented and what each one reports, the required Tracy version and the prebuilt downloads for Windows, Linux, and macOS, enabling `TRACY` in `/etc/default/<service>`, connecting through the profiler's network discovery, headless capture with `tracy-capture`, reading the timeline and finding the expensive stage through the Statistics and Find zone views, and the limits of the current tooling (EDGEAI-1667).
- **Message Schemas section** (`perception/index.md`): the [EdgeFirst Schemas](https://github.com/EdgeFirstAI/schemas) repository, the borrow-based CDR decoding which reads fields at recorded offsets rather than materializing them, and measured decode times for the largest variant of each schema the services publish against Fast-CDR and Cyclone DDS.
- **`lidar/imu` topic** (`perception/topics/{index,lidar}.md`): the inertial readings reported by the LiDAR sensor itself, which fields are populated, and its use by the ground plane filter.
...
- **Perception section reorganized** (`docs/.nav.yml`): the section now follows the reading flow of the product.  The introduction and the Launcher are followed by Topics and Dataset Collection, renamed from "Dataset Collection Middleware", then a new Advanced group holding 4K Video, Profiling the Middleware and Clock Synchronization and Timestamps, then the Developer Guide and API Reference, with Deep View RT last.  No pages were added or removed.
- **Middleware architecture diagram replaced** (`perception/index.md`, `perception/topics/index.md`): the previous Mermaid diagram routed every service through a single `zenoh` node, which reads as a message broker.  The services are Zenoh peers which discover one another through multicast scouting, so both pages now carry a diagram which groups the services by which side of the session they touch, names the topics each publishes and subscribes, and shows the `zenohd` router as the gateway used to reach the topics from another machine along with the Web UI, on-device, remote, and ROS 2 bridge consumers.  Light and dark variants are supplied through the `#only-light` and `#only-dark` convention.
- **Tracy content on the 4K page** (`perception/4k/index.md`): the monitoring section pointed at Tracy 0.12.2, two releases behind the version the services now require, and duplicated the connection walkthrough.  It now describes what the tiling paths report and links to `perception/profiling.md` for the workflow and the version requirement, keeping the profiler screenshots for the 4K context.  The `TRACY` row of the configuration reference (`platforms/configuration/index.md`) links there as well.
- **Configuration section rewritten for the 2026.08 services** (`platforms/configuration/*.md`): service inventory with short unit names under `maivin.target`, `EnvironmentFile` syntax and empty-value semantics, `/usr/etc/default` defaults, and every key of `/etc/default/{camera,model,recorder,radarpub,lidarpub,fusion}` from the shipped `.default` files including `CAMERA_MODE`, 4K tiling, camera calibration and transforms, `DELEGATE`, `TRACK_SCORE`, `CLASSES`, the unified `model/output` topic and legacy topic opt-in, recorder all-topics default and `CUBE_FPS`, radar transforms and Raivin ultra-short defaults, fusion late-fusion inputs, outputs, EdgeFirst Fusion Model, tracking and bins, and LiDAR sensor type, clustering, ground filter, Ouster and Robosense settings.
...
- **LiDAR depth and reflectivity topics** (`perception/topics/lidar.md`, `perception/topics/index.md`, `perception/dev/examples/lidar.md`): `lidar/depth` and `lidar/reflect` are an obsolete feature and are no longer published by the `lidarpub` service.  Their topic sections, the table rows, the two developer examples, and the `lidar_depth.png` and `lidar_reflect.png` screenshots are removed.
