...
=== "Raivin Ultra Short"
    ...
    The Raivin Ultra Short dataset was created by Au-Zone Technologies using a [Raivin platform](../platforms/quickstart/raivin/index.md) with either the base Radar module or a LiDAR module attached to the device to aid in the 3D annotation process.  The dataset contains a single label "person" with 2D (image-based bounding box and instance segmentation) and 3D (real-world bounding boxes) annotations useful for training Object Detection or Fusion models for scene-understanding and spatial-perception tasks.  The dataset samples contains both indoor and rigourous outdoor environments such as construction sites.  The dataset is set to an ultra short range where the field of the objects spans no more than 10 meters away from the Raivin sensor. A Fusion model can be trained in EdgeFirst Studio using this dataset in under 1 hour.  The trained models can be deployed in a field-ready [Raivin Platform](../platforms/quickstart/raivin/index.md) for people awareness tasks.
