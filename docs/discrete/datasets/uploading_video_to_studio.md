# Upload Video 

Video files can be uploaded into any dataset container in EdgeFirst Studio.  Choose the dataset container to upload the video file.  In this case, the dataset is called "Coffee Cup".  Click on the dataset context menu (three dots) and select import.

<figure markdown="span">
![Dataset Import Option](../../datasets/assets/capture/mobile-dataset-import-option.jpg){ align=center }
<figcaption>Dataset Import Option</figcaption>
</figure>

This will bring you to the "Import Dataset" page.

<figure markdown="span">
![Dataset Import](../../datasets/assets/capture/mobile-import-dataset.jpg){ align=center }
<figcaption>Dataset Import</figcaption>
</figure>

Click on the drop-down that says "Import Type" and then specify "Videos" and then click "Done" as shown below. 

<figure markdown="span">
![Dataset Video Import](../../datasets/assets/capture/mobile-video-import-type.jpg){ align=center }
<figcaption>Dataset Video Import</figcaption>
</figure>

Now that the import type is specified to a "Videos", click on "select files" as indicated.

<figure markdown="span">
![Select Video File](../../datasets/assets/capture/mobile-select-video-file.jpg){ align=center }
<figcaption>Select Video File</figcaption>
</figure>

On an android device, this will bring up the option to specify the location of the files.

<figure markdown="span">
![Android Select File Options](../../datasets/assets/capture/mobile-video-options.jpg){ align=center }
<figcaption>Android Select File Options</figcaption>
</figure>

In my current setup, I have selected "My Files" from the options above and then "Videos" which will allow me to pinpoint the location of the video I have recorded. 

<figure markdown="span">
![Android File Manager](../../datasets/assets/capture/mobile-video-location.jpg){ align=center }
<figcaption>Android File Manager</figcaption>
</figure>

Once the video file has been selected, set the desired FPS (frames per second), and then go ahead and click the "Start Import" button to start importing the video file. 

<figure markdown="span">
![Import Fields](../../datasets/assets/capture/mobile-video-import-fields.jpg){ align=center }
<figcaption>Import Fields</figcaption>
</figure>

This will start the import process and once it is completed, you should see the number of images in the dataset increased.  If you do not see any changes, refresh the browser. 

<figure markdown="span">
![Imported Video](../../datasets/assets/capture/imported-video-outcome.jpg){ align=center }
<figcaption>Imported Video</figcaption>
</figure>
