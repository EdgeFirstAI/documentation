...
1. Load the model specifying the execution provider using the device's inference engine.
    ...
    !!! note "Execution Providers"
        The device NPU can be specified with `['NnapiExecutionProvider', 'VsiNpuExecutionProvider']`.
...
Next, you can also download this [sample TFLite model](assets/yolo11s-seg.onnx){: download="yolo11s-seg.onnx"} for running the demo. Otherwise, update the path to your own YOLO segmentation TFLite model in the python script. 
