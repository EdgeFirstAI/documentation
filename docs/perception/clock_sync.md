...
A single published message is stamped in three places. Two of them must be equal; the third is deliberately different.
...
| Zenoh sample timestamp | Zenoh transport metadata attached on `put()` | The publishing service | Must equal `header.stamp` to the nanosecond |
...
flowchart TD
    ...
    B -->|"same nanosecond value"| C["Zenoh sample timestamp<br/>put(..., timestamp)"]
...
`log_time - publish_time` on a recording is the end-to-end latency from acquisition to the Recorder: sensor readout, encoding, inference, Zenoh transport and scheduling. On a healthy system it is a few milliseconds for sensors and tens of milliseconds for model outputs. It must never be days.
...
- 4K tile stitching is the one place `header.stamp` is read: the four tiles of a frame are grouped by exact equality of their payload stamps. This works only because all four tiles carry the identical acquisition stamp.
...
The Maivin can act as an IEEE 1588 grandmaster on the internal sensor network (`ethernet1`) using `ptp4l` with hardware timestamping and `phc2sys-master@ethernet1`, which pushes `CLOCK_REALTIME` into the interface's PTP hardware clock with a zero UTC-TAI offset, so sensors receive UTC. This is installed but **disabled by default**; enable it per deployment when a sensor should timestamp in the host clock domain:

```shell
sudo systemctl enable --now ptp4l.service phc2sys-master@ethernet1.service
```

With PTP running, the Ouster lidar can be configured with `TIMESTAMP_MODE="ptp1588"` so its per-column measurement timestamps are Unix time in the host domain. Without PTP the sensor's `internal` mode counts nanoseconds since sensor power-on, which is not a host-domain clock and must not be published as `header.stamp`.

## Service Reference

The table records, per topic, where acquisition time comes from and what the contract requires for `header.stamp` and the Zenoh sample timestamp. Topic names are shown without the hostname namespace prefix.

| Service | Topic | Acquisition source | `header.stamp` | Zenoh sample timestamp |
| --- | --- | --- | --- | --- |
| camera | `camera/frame`, `camera/dma`, `camera/h264`, `camera/jpeg`, `camera/h264/{tl,tr,bl,br}` | V4L2 buffer timestamp (`CLOCK_MONOTONIC`, end of frame) | Monotonic capture time plus the current realtime offset | Same as `header.stamp` |
...
| model | `model/output`, `model/boxes2d`, `model/info`, `model/visualization` | The input camera frame | Camera frame stamp | Same as `header.stamp` |
| model | `model/mask`, `model/mask_compressed` | The input camera frame | No header in the schema | Camera frame stamp |
| fusion | `fusion/boxes3d`, `fusion/radar`, `fusion/occupancy` | The radar or lidar point cloud | Point cloud stamp | Same as `header.stamp` |
| fusion | `model/.../tracked` masks | The radar cube used for the grid | No header in the schema | Radar cube stamp |
...
Foxglove orders and plays messages by receive time (`log_time`) by default and can be switched to header stamps in its playback settings. To check the contract directly, compare the two MCAP stamps per topic:
...
- Timestamps compare across devices only if every device is synchronized. Use the GNSS reference where available and NTP otherwise; cross-device error is then bounded by chrony's reported root dispersion.
