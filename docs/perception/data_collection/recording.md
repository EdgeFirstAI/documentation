...
- Discovers and records every topic published on the device, or only the topics selected in its [configuration](../../platforms/configuration/mcap_recording.md)
...
The topics are recorded with the device [hostname namespace](../topics/index.md#hostname-namespaces) stripped, so the channels in the MCAP file are named `/camera/h264`, `/model/output`, and so on.  The zero-copy `camera/frame` topic is not recorded, the camera pixels are recorded from the `camera/h264` stream.
...
Underneath the MCAP navbar, we have the current MCAP recording directory, which is `/home/torizon/recordings` by default or `/media/DATA` when an SD card is configured as the [storage location](../../platforms/configuration/mcap_recording.md#storage-location).  Under this directory text, we have the "Search files..." textfield if we need to search for a specific file.
...
- The Action buttons, which are:
    ...
    - The "Upload" button, which uploads the MCAP file to EdgeFirst Studio as described in [Publishing](publishing.md#method-1-upload-from-the-web-ui).
...
!!! warning
     ...
     Having the Recording Service run for prolonged periods of time will fill the storage of the Raivin.  Use this functionality with caution and consider setting a [maximum duration](../../platforms/configuration/mcap_recording.md#duration).
