...
```
Which should output four files:
```
...
```
These wheel files (as well as the Python topics scripts) can be copied onto the device via SSH secure copy.  Once there, they can be installed in a virtual enviornment:
```bash
$ python -m venv venv
$ source venv/bin/activate
...
```
Or start the service from the [Raivin's WebUi](../../platforms/walkthrough.md):
