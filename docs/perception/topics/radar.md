...
The radar topics are managed by the `radarpub` service and handles interfacing with a conencted radar to produce radar point clouds and radar cubes.  The service can also window radar points and cluster radar points based on their proximity to each other.  The following is a list of key features provided by the radar service.
