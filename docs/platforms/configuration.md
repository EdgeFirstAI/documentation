...
#### Visualization
Enables publishing the legacy visualization message for detection models.  It is 'false' by default which means only the primary model box and mask topics will be available.  If using [Foxglove Studio](foxglove.md) make sure you install the [EdgeFirst for Foxglove Plug-in](./foxglove.md#installing-edgefirst-plugin).
...
## LiDAR Configuration (Raivin-only)
