...
The models shipped with the device are installed under `/usr/share/edgefirst/modelzoo/` for the vision model and `/usr/share/edgefirst/fusion/` for the EdgeFirst Fusion Models.  These directories are part of the read-only operating system image, uploaded models are stored under the `torizon` user home directory instead.
